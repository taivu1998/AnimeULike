...
class DropoutGNet(nn.Module):
    def __init__(self, **kwargs):
        ...
        if not "hidden_dims" in kwargs: self.hidden_dims=[500]
        #LAYERS
        ...
        for (i,hidden_dim) in enumerate(self.hidden_dims):            
            ...
            hidden_u=nn.Linear(prev_dim if i else self.latent_dim,hidden_dim)
        ...
        if self.num_gnn_feats==1:
            self.num_gnn_feats=0
    ...
    def forward(self,u,v,u_phi,v_phi,u_inds,v_inds,y,eval=False):
        ...
        v_mask=torch.rand_like(v[:,:1])<self.eip if eval else torch.rand_like(v)<self.item_drop_p
        v=v_mask*v
...
assert item_vectors.shape[1]==200
...
all_u_transforms=[np.average(v[pref_mask.iloc[i].values],axis=0) for i in range(len(user_df))]
...
if results.mode=="tfidf":
    path="cached/anime2vec/tfidf/"
    VAR_NAMES=['tfidfu_feats_svd_std','tfidfv_feats_svd_std','tfidfval_v_feats_svd_std']
    for j in VAR_NAMES:
        with open(path+j+".json") as f:
            dic=json.load(f)
            locals()[j]={int(float(k)):np.array(dic[k]) for k in dic}
            
    u_feats,v_feats,val_v_feats=tfidfu_feats_svd_std,tfidfv_feats_svd_std,tfidfval_v_feats_svd_std

elif results.mode=="bert-pretrained":
    ...
    raise AssertionError("not valid mode")
...
class UserAnimePermuteID(Dataset):
    ...
    def by_id(self, u_id,v_id):
        ...
        if random.random()<self.user_transform_p: 
            u=u_transforms[u_id]
...
class UserAnimeID(Dataset):
    def __init__(self,u_dict,v_dict,u_feats,v_feats,user_transform_p=0.5):
        self.V_dict=v_dict
        self.U_dict=u_dict
        self.V_feats=v_feats 
        self.U_feats=u_feats
...
class UserAnimeCacheID(UserAnimeID):
    def __init__(self,u_dict,v_dict,u_feats,v_feats,user_transform_p=0.5):
        super().__init__(u_dict,v_dict,u_feats,v_feats,user_transform_p=user_transform_p)
...
config={
    "mode":[results.mode],
    "latent_dim":[200],#FIX
    "anime_dim":[300 if results.mode=="tfidf" else 774],#FIX
    "num_gnn_feats":[0],
    
    "user_transform_p":[0.0],    
    "item_drop_p":[0.5],
    "eup":[0.0,1.0],
    "eip":[0.0,1.0],
    "activation":["tanh"],
...
def cold_metric(n_users,n_items,config=None,dn=None):
    ...
    cache_uaid=UserAnimeCacheID(u_dict,val_v_dict,u_feats,val_v_feats,user_transform_p=dn.eup)
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=25,metric_every=METRIC_EVERY,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        # val_uaid=UserAnimePermuteID(u_dict,val_v_dict,u_feats,val_v_feats,pos_thresh=config['pos_thresh'],neg_thresh=config['neg_thresh'],sample_mask=val_sample_dic,val=True)
...
run_experiments(configs,"models/recsys/anime_cold")
