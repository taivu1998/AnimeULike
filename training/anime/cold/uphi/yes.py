...
class DropoutGNet(nn.Module):
    ...
    def forward(self,u,v,u_phi,v_phi,u_inds,v_inds,y,eval=False):
        ...
        v=v_mask*v
        u_mask=torch.rand_like(u[:,:1])<self.eup if eval else torch.rand_like(u)<self.user_drop_p
        u=u_mask*u
...
class UserAnimePermuteID(Dataset):
    def __init__(self,u_dict,v_dict,u_feats,v_feats,n_scores_user=25,pos_neg_ratio=5):
