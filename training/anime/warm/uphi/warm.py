...
class DropoutGNet(nn.Module):
    ...
    def forward(self,u,v,u_phi,v_phi,u_inds,v_inds,y,eval=False):
        ...
        v_mask=torch.rand_like(v)<(self.eip if eval else self.item_drop_p)
        v=v_mask*v
...
np.save("cached/anime2vec/trained/v_dict.npy",v_dict)
...
np.save("cached/anime2vec/trained/v_feats.npy",v_feats)
...
def cold_metric(n_users,n_items,config=None,dn=None,save_path=None):
    global best_ir
    ...
    if ir > best_ir:
        best_ir=ir
        print("saving with ir:",ir)
        np.save("cached/anime2vec/trained/f_v_cache.npy",V_hat.cpu())
        if save_path:
            torch.save(dn.state_dict(),"{}.pt".format(save_path))
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        for (i,data) in tqdm(enumerate(dload,0)):
            if i%metric_every==0:
                ...
                metrics=cold_metric(0,len(v_dict),dn=dn,save_path=save_path)
