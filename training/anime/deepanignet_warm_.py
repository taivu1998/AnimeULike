...
user_path="{}/10000_warm_user_factors.csv".format(ANIME_PATH)
...
pref_matrix,val_pref_matrix=pd.read_csv("{}/10000_warm_train.csv".format(ANIME_PATH),index_col=0),pd.read_csv("{}/10000_warm_val.csv".format(ANIME_PATH),index_col=0)
pref_mask=pref_matrix>0.0
val_pref_mask=val_pref_matrix>0.0
warm_user_factors=pd.read_csv("{}/10000_warm_user_factors.csv".format(ANIME_PATH),index_col=0)
warm_item_factors=pd.read_csv("{}/10000_warm_item_factors.csv".format(ANIME_PATH),index_col=0)
...
all_v_feats=[rec.getanime(i) for i in range(len(rec.data))]
v_feats=dict(zip(train_ids,all_v_feats))
assert len(all_v_feats)==9000
all_u_transforms=[np.average(warm_item_factors[pref_mask.iloc[i].values],axis=0) for i in range(len(user_df))]
...
def get_u_feat(i):
    ...
    u_attention_mask = []
    u_scalars = []
    item_inds=np.argwhere(pref_mask.iloc[i].values).flatten()
    if not len(item_inds): item_inds=[np.random.choice(pref_mask.shape[1])]
    for j in np.random.permutation(item_inds):
...
if os.path.isfile("cached/recsys/u_feats_warm.npy"):
    u_feats=np.load("cached/recsys/u_feats_warm.npy",allow_pickle=True).item()
else:
    all_u_feats=[get_u_feat(i) for i in range(len(user_df))]
    u_feats=dict(zip(user_ids,all_u_feats))
    np.save("cached/recsys/u_feats_warm.npy",u_feats)


print("Feats computed")

assert len(v_dict)==9000
...
def cold_metric(n_users,n_items,config=None,dn=None):
    ...
    V_hat=torch.stack([dn.dropoutgnet.f_v_cache[k] for k in v_dict])
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=25,metric_every=50,save_path=""):
...
def run_experiments(configs,path="models/recsys/tfidf/recs"):
    ...
    for config in configs:
        ...
        run_experiment(dn,optimizer,1000,plot_every=3,save_path=save_path)


run_experiments(configs,"models/recsys/deepanignet_warm")
