...
parser.add_argument('-i', action='store',
                    dest='num_iters_to_freeze',
                    help='number of iters (times metric is computed) before freezing')                     
...
device = "cuda:0"
NUM_ITERS_TO_FREEZE=int(results.num_iters_to_freeze)
...
def get_u_feat(i,no_shows=max_shows_per_user):
    u_input_ids = []
    u_attention_mask = []
    u_scalars = []
    for j in np.random.permutation(np.argwhere(pref_mask.iloc[i].values).flatten()):
        ...
        if len(u_input_ids) >= no_shows:
            break

    num_shows_per_user = len(u_input_ids)
    if num_shows_per_user < no_shows:
        u_input_ids = u_input_ids + (no_shows - num_shows_per_user) * [empty_input_ids]
        u_attention_mask = u_attention_mask + (no_shows - num_shows_per_user) * [empty_attention_mask]
        u_scalars = u_scalars + (no_shows - num_shows_per_user) * [empty_scalars]
...
def sample_u_feats(no_shows=max_shows_per_user):
    all_u_feats=[get_u_feat(i,no_shows) for i in range(len(user_df))]
...
if os.path.isfile("cached/recsys/u_feats.npy"):
    master_u_feats=np.load("cached/recsys/u_feats.npy",allow_pickle=True).item()
else:
    all_u_feats=[get_u_feat(i) for i in range(len(user_df))]
    master_u_feats=dict(zip(user_ids,all_u_feats))
...
TRAIN_POS,TRAIN_NEG,VAL_POS,VAL_NEG=50000,50000,5000,5000
...
config={
    ...
    "basket_size":[int(results.no_shows)],
    "freeze_iter":[NUM_ITERS_TO_FREEZE]
}
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
def get_optimizer(dn):
    ...
    optimizer=optim.SGD([{'params': fine_tune_dropoutnet_params},
                          {'params': fine_tune_bert_params, 'lr': 0.005/NUM_ACCUMULATION_STEPS}
                        ], lr=0.005/NUM_ACCUMULATION_STEPS)
    return optimizer

#@title Learning rate adjust
def adjust_learning_rate(optimizer, epoch,rate=0.9):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    optimizer.param_groups[1]['lr'] *= LR_DECAY_RATE
    # for param_group in optimizer.param_groups:
    #     param_group['lr'] = lr

def freeze(dn,optimizer):
    fine_tune_bert_params=list(map(lambda x:x[1],filter(lambda x:x[0] in fine_tune_bert_param_names,dn.named_parameters())))
    for param in fine_tune_bert_params: param.requires_grad=False 
    dn.frozen=True
...
def cold_metric(n_users,n_items,config=None,dn=None):
    ...
    cache_uaid=UserAnimeCacheID(u_dict,val_v_dict,master_u_feats,val_v_feats)
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=25,metric_every=METRIC_EVERY,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        u_feats=sample_u_feats(int(results.no_shows))
        uaid=UserAnimeIDRandomAccess(u_dict,v_dict,u_feats,v_feats,pos_thresh=config['pos_thresh'],neg_thresh=config['neg_thresh'],sample_mask=sample_dic)
        dload=DataLoader(uaid,batch_size=BATCH_SIZE,shuffle=True,num_workers=NUM_WORKERS)
        val_uaid=UserAnimeIDRandomAccess(u_dict,val_v_dict,u_feats,val_v_feats,pos_thresh=config['pos_thresh'],neg_thresh=config['neg_thresh'],sample_mask=val_sample_dic,val=True)
        ...
        for (i,data) in tqdm(enumerate(dload,0)):
            if i%metric_every==0:
                ...
                if len(open("{}_metrics.txt".format(save_path)).readlines())==NUM_ITERS_TO_FREEZE:
                    freeze(dn,optimizer)
            u, v, u_input_ids, u_attention_mask, u_scalars, num_shows, v_input_ids, v_attention_mask, v_scalars, u_ids,v_ids, y = data
            dn.update(v_ids)   
            v=np.zeros_like(v)
...
run_experiments(configs,"models/recsys/deepanignet_freeze")
