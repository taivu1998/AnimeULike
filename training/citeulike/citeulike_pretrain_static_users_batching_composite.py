...
class CiteULikeNegSample(CiteULikePosOnly):
    def __init__(self,data,u_dict,v_dict,u_feats,v_feats,neg_thresh=LABELS_MEAN-LABELS_STD,pos_thresh=LABELS_MEAN+LABELS_STD,pos_neg_ratio=1.0,row_major=True):
        super().__init__(data,u_dict,v_dict,u_feats,v_feats)
        self.num_neg=int(len(self.data)*pos_neg_ratio)
        self.neg_data=UserAnimePermuteID(u_dict,v_dict,u_feats,v_feats,neg_thresh=neg_thresh,pos_thresh=dn.pos_thresh,n_scores_user=self.num_neg//self.num_users)
        ...
        self.partial_shuffle=np.random.permutation(len(self.data))

    def __getitem__(self,idx):
        #this first exhausts pos samples from super().__getitem__() before 
        #going to negative
        if idx<len(self.data):
            return super().__getitem__(self.partial_shuffle[idx])
        else:
            idx-=len(self.data)
            return self.neg_data.__getitem__(idx)
            
      
    def __len__(self): 
        return self.neg_data.__len__()+len(self.data)



class UserAnimePermuteID(Dataset):
    def __init__(self,u_dict,v_dict,u_feats,v_feats,neg_thresh=float("inf"),pos_thresh=float("-inf"),n_scores_user=25):
        ...
        self.item_permute=[np.random.permutation(np.array(self.item_ids)[np.argwhere(self.labels[i]>pos_thresh or self.labels[i]<neg_thresh).flatten()]) for i in range(len(self.user_ids))]
        print(np.average([len(self.labels[i]<neg_thresh) for i in range(len(self.labels))]),"number of neg items, on avg")
        print(np.average([len(self.labels[i]>pos_thresh) for i in range(len(self.labels))]),"number of pos items, on avg")
        self.inv_user_ids=dict(zip(self.user_ids,range(len(self.user_ids))))
        self.inv_item_ids=dict(zip(self.item_ids,range(len(self.item_ids))))
    ...
    def __getitem__(self,idx):        
        ...
        if len(self.item_permute[user_idx]):
            permuted_item_id=self.item_permute[user_idx][item_idx%len(self.item_permute[user_idx])]
        else: 
            permuted_item_id=np.random.choice(self.item_ids)
...
config={#printed to save the model
    ...
    "item_drop_p":[0.5],
    "activation":["tanh"],
    "pos_neg_ratio":[1.0,10.0],
    "mode":[mode],#to make lookup easier,
    "neg_thresh":[round(LABELS_MEAN-LABELS_STD)],
    "pos_thresh":[0]
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=1,metric_every=500*NUM_ACCUMULATION_STEPS,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        uaid=CiteULikeNegSample(citeulike_data,u_dict,v_dict,u_feats,v_feats,neg_thresh=dn.neg_thresh,pos_thresh=dn.pos_thresh,pos_neg_ratio=dn.pos_neg_ratio)
        dload=DataLoader(uaid,shuffle=True,batch_size=BATCH_SIZE,num_workers=NUM_WORKERS)
        val_uaid=CiteULikeNegSample(citeulike_val_data,u_dict,val_v_dict,u_feats,val_v_feats,neg_thresh=dn.neg_thresh,pos_thresh=dn.pos_thresh,pos_neg_ratio=dn.pos_neg_ratio)
        val_dload=DataLoader(val_uaid,shuffle=True,batch_size=BATCH_SIZE,num_workers=NUM_WORKERS)
        dn.train(True)
...
run_experiments(configs,"models/citeulike/uphi_users_batching_composite/{}".format(mode))
