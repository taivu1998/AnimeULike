...
EMBEDDING_PATH_1="cached/citeulike/tfidf/"
EMBEDDING_PATH_2="cached/citeulike/bert-pretrained/"
...
for j in VAR_NAMES:
    with open(EMBEDDING_PATH_1+j+".json") as f:
        dic=json.load(f)
        globals()[j]={int(float(k)):np.array(dic[k]) for k in dic}
for j in VAR_NAMES:
    with open(EMBEDDING_PATH_2+j+".json") as f:
        dic=json.load(f)
        globals()[j]={int(float(k)):np.append(np.array(dic[k]),globals()[j][int(float(k))]) for k in dic}
...
class CiteULikeNegSample(CiteULikePosOnly):
    def __init__(self,data,u_dict,v_dict,u_feats,v_feats,neg_thresh=0.0,pos_neg_ratio=0.0,
    user_transform_p=0.5,row_major=True):
...
def cold_metric(n_users,n_items,config=None,dn=None):
    ...
    cache_uaid=UserAnimeCacheID(u_dict,val_v_dict,u_feats,val_v_feats,user_transform_p=dn.user_transform_p)
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=1,metric_every=500*NUM_ACCUMULATION_STEPS,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        uaid=CiteULikeNegSample(citeulike_data,u_dict,v_dict,u_feats,v_feats,neg_thresh=dn.neg_thresh,pos_neg_ratio=dn.pos_neg_ratio,user_transform_p=dn.user_transform_p)
        dload=DataLoader(uaid,batch_size=BATCH_SIZE,shuffle=True,num_workers=NUM_WORKERS)
        val_uaid=CiteULikeNegSample(citeulike_val_data,u_dict,val_v_dict,u_feats,val_v_feats,user_transform_p=dn.user_transform_p)
...
run_experiments(configs,"models/citeulike/no_uphi/comp_vphi/")
