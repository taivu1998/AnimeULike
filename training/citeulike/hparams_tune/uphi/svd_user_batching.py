...
#@title Load cache
import json
VAR_NAMES=['u_dict','v_dict','val_v_dict','u_feats','v_feats','u_transforms','u_feats_svd_avg_std','v_feats_svd_std','val_v_feats_svd_std']
for j in VAR_NAMES:
...
def resample_u_feats(basket_size):
    u_fe=[np.average(v_np[np.random.choice(np.argwhere(train_pref_matrix[i]>0).flatten(),min(len(np.argwhere(train_pref_matrix[i]>0)),basket_size))],axis=0) for i in range(train_pref_matrix.shape[0])]
    return dict(zip(u_feats.keys(),prep_standardize_dense(randomized_svd(np.array(u_fe),n_components=300,n_iter=5)[0])[1]))
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=1,metric_every=500*NUM_ACCUMULATION_STEPS,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        uaid=UserAnimePermuteID(u_dict,v_dict,resample_u_feats(dn.basket_size),v_feats,n_scores_user=dn.n_score_users,pos_neg_ratio=dn.pos_neg_ratio)
...
run_experiments(configs,"models/citeulike/uphi/svd_batching/hparam_search/{}".format(mode))
