...
def get_u_feat(i,max_shows_per_user=10):
    u_input_ids = []
    u_attention_mask = []
...
class CiteULikeNegSample(CiteULikePosOnly):
    def __init__(self,data,u_dict,v_dict,u_feats,v_feats,neg_thresh=-2.0,pos_neg_ratio=1.0,row_major=True):
...
def cold_metric(n_users,n_items,config=None,dn=None):
    ...
    u_feats=[get_u_feat(i,10) for i in range(len(user_df))]
    u_feats_train = [u_feats[i] for i in train_user_ids]
    u_feats=dict(zip(train_user_ids,u_feats_train))
...
def train(rank,num_epochs=100,cont=-1,num_accumulation_steps=1,save_every=1,metric_every=500,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        dload=DataLoader(uaid,batch_size=BATCH_SIZE//len(DEVICE_IDS),shuffle=True,num_workers=NUM_WORKERS)
        val_uaid=CiteULikeNegSample(citeulike_val_data,u_dict,val_v_dict,u_feats,val_v_feats)
        val_dload=DataLoader(val_uaid,batch_size=BATCH_SIZE//len(DEVICE_IDS),shuffle=True,num_workers=NUM_WORKERS)
        ...
        for (i,data) in tqdm(enumerate(dload,0)):
            ...
            if i%metric_every==metric_every-1:
...
def run_experiment(rank,num_epochs=100,plot_every=10,save_path=""):
    ...
    for _ in range(num_epochs):
        ...
        for (preds,t_l,v_l) in train(rank,num_epochs,save_path=save_path):
            preds=preds
...
def run_experiments(rank,world_size,configs,path="models/citeulike/bert/recs"):
    ...
    for config in configs:
        ...
        run_experiment(rank,250,plot_every=3,save_path=save_path)

if __name__ == "__main__":
    world_size=len(DEVICE_IDS)
    mp.spawn(run_experiments,args=(world_size,configs,"models/citeulike/bert/bert_finetune"),nprocs=world_size,join=True)
