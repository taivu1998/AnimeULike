...
device = "cuda:0"
BATCH_SIZE,NUM_WORKERS=20,10
NUM_ACCUMULATION_STEPS=5
LR_DECAY_RATE=0.99
NUM_BUCKETS=0
LIBRARY_PATH="cached/citeulike/u_feats_library/bert_pretrained_finetune"
...
# master_all_u_feats=[get_u_feat(i,999) for i in range(len(user_df))]
# master_all_u_feats_train = [master_all_u_feats[i] for i in train_user_ids]
# master_u_feats=dict(zip(train_user_ids,master_all_u_feats_train))
...
master_u_feats=np.load(path+"pretrained_u_feats.npy",allow_pickle=True).item()
print("LOADED MASTER U FEATS")
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,lr_decay_rate=LR_DECAY_RATE,u_feats_samples=NUM_BUCKETS,save_every=1,metric_every=500*NUM_ACCUMULATION_STEPS,save_path=""):
    ...
    if os.path.isfile("{}_{}.pt".format(save_path,num_epochs-1)): 
        ...
        if chkpts[indmax]:
            ...
            latest_ckpt="{}_{}.pt".format(save_path,save_every*indmax-1+save_every)
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        uaid=CiteULikeNegSample(citeulike_data,u_dict,v_dict,[master_u_feats],v_feats,neg_thresh=dn.dropoutnet.neg_thresh,pos_neg_ratio=dn.dropoutnet.pos_neg_ratio)
        dload=DataLoader(uaid,batch_size=BATCH_SIZE,shuffle=True,num_workers=NUM_WORKERS)
        val_uaid=CiteULikeNegSample(citeulike_val_data,u_dict,val_v_dict,[master_u_feats],val_v_feats)
...
run_experiments(configs,"models/citeulike/bert/bert_pretrained_finetune")
