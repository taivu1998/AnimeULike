...
class DropoutNet(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.feats_norm=nn.BatchNorm1d(self.anime_dim)#contains learnable param
        self.latent_norm=nn.BatchNorm1d(self.latent_dim)
    ...
    def forward(self,u,u_transform,v,u_phi,v_phi,u_inds,v_inds,y,b=True,eval=False):
        ...
        u,v=self.latent_norm(u),self.latent_norm(v)
        u_phi,v_phi=self.feats_norm(u_phi),self.feats_norm(v_phi)
        
        # if not b:
        #     u=u_transform#happens with eval too
        ...
        if eval:
            ...
            if b:
                v_mask=torch.rand_like(v)<self.drop_p
                v=v_mask*v
...
class CiteULikePosOnly(Dataset):
    def __init__(self,data,u_dict,v_dict,u_feats,v_feats):
        ...
        self.labels=(raw_labels- np.mean(raw_labels)) / (np.std(raw_labels)*10)
...
config={#printed to save the model
    ...
    "neg_thresh":[round(LABELS_MEAN+k*LABELS_STD,4) for k in np.linspace(-1.0-1.0,1)]
}
