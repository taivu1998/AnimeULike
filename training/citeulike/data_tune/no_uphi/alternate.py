...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=1,metric_every=500*NUM_ACCUMULATION_STEPS,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        uaid=UserAnimePermuteID(u_dict,v_dict,u_feats,v_feats,n_scores_user=dn.n_score_users,pos_neg_ratio=dn.pos_neg_ratio,user_transform_p=dn.user_p)
        ...
        val_uaid=UserAnimePermuteID(u_dict,val_v_dict,u_feats,val_v_feats,dn.n_score_users,pos_neg_ratio=dn.pos_neg_ratio,user_transform_p=dn.user_p)
        ...
        for (i,data) in tqdm(enumerate(dload,0)):
            ...
            u,v,u_phi,v_phi,u_ids,v_ids,y=data
            change_mask=torch.rand_like(u_ids.float())<dn.user_p
            u[change_mask]=torch.tensor([u_transforms[ch.item()] for ch in u_ids[change_mask]]).double() if i%2 else torch.zeros_like(u[change_mask])
            a,b,c,d=torch.tensor(u,dtype=torch.float),torch.tensor(v,dtype=torch.float),torch.tensor(u_phi,dtype=torch.float),torch.tensor(v_phi,dtype=torch.float)
...
run_experiments(configs,"models/citeulike/no_uphi/alternate/in_and_out/{}".format(mode))
