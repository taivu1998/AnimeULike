...
class DropoutNet(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        assert {"drop_p","latent_dim","hidden_dims"}.issubset(set(kwargs.keys()))    
        ...
        # self.latent_embed_u=nn.Linear(self.latent_dim,self.latent_dim)
        # self.latent_embed_v=nn.Linear(self.latent_dim,self.latent_dim)        

        #LAYERS
        prev_dim=self.latent_dim+self.anime_dim
        # self.input_norm_u=nn.BatchNorm1d(self.latent_dim)
        # self.input_norm_v=nn.BatchNorm1d(prev_dim)
        for (i,hidden_dim) in enumerate(self.hidden_dims):            
            hidden_v=nn.Linear(prev_dim,hidden_dim)
            nn.init.normal_(hidden_v.weight,std=0.01)
            nn.init.zeros_(hidden_v.bias)
            if not i: prev_dim=self.latent_dim
    ...
    def forward(self,u,u_transform,v,u_phi,v_phi,u_inds,v_inds,y,b=True,eval=False):
        #b: whether to dropout vs transform
        
        B=u.shape[0]
        
        if not b:
            u_mask=torch.rand_like(u[:,0])<self.drop_p
            u[u_mask[:,0]]=u_transform[:,0]#happens with eval too
        ...
        if eval:
            ...
            if b:
                v_mask=torch.rand_like(v[:,:1])<self.drop_p
                v=v_mask*v
...
#@title Load cache
import json
VAR_NAMES=['u_dict','v_dict','val_v_dict','u_transforms','u_feats','v_feats','val_v_feats']
for j in VAR_NAMES:
...
class CiteULikePosOnly(Dataset):
    ...
    def __getitem__(self,idx): 
        ...
        return (u,u_transforms[u_id],v,u_phi,v_phi)+(u_id,v_id,)+(y,)
...
class CiteULikeNegSample(CiteULikePosOnly):
    def __init__(self,data,u_dict,v_dict,u_feats,v_feats,neg_thresh=0.0,pos_neg_ratio=0.0,row_major=True):
        ...
        if num_neg>=len(self.neg_inds):
            self.neg_inds=self.neg_inds.repeat(num_neg//len(self.neg_inds)+1)
...
config={#printed to save the model
    ...
    "neg_thresh":[round(LABELS_MEAN-LABELS_STD,4)]
}
...
def evaluate(epoch,dn,val_dload,preds=None,tru=None,eval=False):
    ...
    with torch.no_grad():
        ...
        for (j,data) in tqdm(enumerate(val_dload,0)):
            ...
            u,u_transforms,v,u_phi,v_phi,u_ids,v_ids,y=data
            v=np.zeros_like(v)
            ...
            u_transforms=torch.tensor(u_transforms,dtype=torch.float)
            z=torch.tensor(y,dtype=torch.float)
            u_ids,v_ids=u_ids.numpy(),v_ids.numpy()
            a,u_transforms,b,c,d,z=a.to(device),u_transforms.to(device),b.to(device),c.to(device),d.to(device),z.to(device)
            r,loss=dn.forward(a,u_transforms,b,c,d,u_ids,v_ids,z,b=j%2,eval=eval)
...
def train(dn,optimizer,num_epochs=100,cont=-1,num_accumulation_steps=NUM_ACCUMULATION_STEPS,save_every=1,metric_every=500*NUM_ACCUMULATION_STEPS,save_path=""):
    ...
    for epoch in range(cont+1,num_epochs):
        ...
        dload=DataLoader(uaid,batch_size=BATCH_SIZE,shuffle=False,num_workers=NUM_WORKERS)
        ...
        for (i,data) in tqdm(enumerate(dload,0)):
            ...
            u_ids,v_ids=u_ids.numpy(),v_ids.numpy()
            a,u_transforms,b,c,d,z=a.to(device),u_transforms.to(device),b.to(device),c.to(device),d.to(device),z.to(device)
            r,loss=dn.forward(a,u_transforms,b,c,d,u_ids,v_ids,z,b=i%2)
...
run_experiments(configs,"models/citeulike/no_uphi/{}".format(mode))
